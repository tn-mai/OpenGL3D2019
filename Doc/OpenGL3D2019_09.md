...
+    meshBuffer.GetFile("Bikuni"), "Player", 20, startPos);
...
MainGameScene.cppを開き、Actor.hのインクルード文の下に、次のプログラムを追加してください。
...
+    const Mesh::FilePtr mesh = meshBuffer.GetFile("oni_small");
...
+      handler(a, acgtorB, actorB->colWorld.center);
+      if (a->health <= 0) {
+        break;
+      }
+    }
...
+        handler(actorA, acgtorB, actorB->colWorld.center);
+        if (actorA->health <= 0) {
+          break;
+        }
+      }
...
```diff
   ...
   player = std::make_shared<StaticMeshActor>(
     meshBuffer.GetFile("Bikuni"), "Player", 20, startPos);
+  player->colLocal = Sphere{ glm::vec3(0), 0.5f };
...
```diff
       ...
       StaticMeshActorPtr p = std::make_shared<StaticMeshActor>(
         mesh, "Kooni", 13, position, rotation);
+      p->colLocal = Sphere{ glm::vec3(0), 1.0f };
       enemies.Add(p);
     }
   }
```
