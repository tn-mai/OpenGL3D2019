...
それから、プロジェクトのプロパティを開いて「リンカー→入力」を選択します。そして、プロパティウィンドウ上部の表示が「すべての構成」と「すべてのプラットフォーム」になっていることを確認します(違っていたらそうなるように変更してください)。それから、右側の「追加の依存ファイル」の欄をクリックして「opengl32.lib」というライブラリ名を追加し、「OK」ボタンを押して変更を確定します。

### 1.2 GLFWEW.hとGLFWEW.cppの追加

GLFWやGLEWの初期化には、以前作成したGLFWEW::Windowクラスが使えます。
エクスプローラーなどを使って、2018年後期のテキストで作成したプロジェクトからGLFWEW.hとGLFWEW.cppをコピーし、現在作成しているプロジェクトのSrcフォルダに貼り付けてください。
それができたらVisual Studioに戻り、ソリューションエクスプローラーの「ソースファイル」フィルタを右クリックして「追加->既存の項目」を選択します。
そして、さきほど追加したGLFWEW.hとGLFWEW.cppを選択して追加してください(Ctrlキーを押しながらファイルを左クリックすると複数のファイルを選択できます)。

>［補足］過去のテキストは`https://github.com/tn-mai/OpenGL3D2018/`にて公開しています。このテキストから始めた場合はここから取得してください。
...
ゲームパッドの状態を取得したり更新したりするにはGLFWを使います。そのため、この機能はGLFWEW::Windowクラスに追加するのがいいでしょう。
...
if ((gamepad.button & GamePad::A) || window.IsKeyPressed(GLFW_KEY_A)) {
  // Aボタンが押されたときの処理.
}
```
...
>［補足］ゲームパッドに割り当てられていないキーについては、これまでどおりIsKeyPressed関数で判定します。
...
タイトル画面以外のすべてのシーンについて、タイトル画面と同様に、ゲームパッド入力によってシーンを切り替えるようにしてください。<br>メインゲーム画面からは２つのシーンに切り替わりますので、STARTボタンでステータス画面へ、Xボタンでゲームオーバー画面へ切り替わるようにしてください。この変更によってflag変数は不要になるので削除しておいてください。なお、キーボードでゲームパッドのXボタンに相当するのはUキーです。ゲームパッドボタンとキーボードの割り当てについてはUpdateGamePad関数のキーボードの処理を参照してください)。
</div>
<div style="page-break-after: always"></div>
...
テキストが長くなってしまいましたので、少し中途半端ですが今回はここまでにしましょう。<br>
