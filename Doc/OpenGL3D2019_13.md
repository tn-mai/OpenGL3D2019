...
通常のfor文では最初にiをインクリメントするように書きますが、vectorから要素を削除する場合は違う書き方をしなければなりません。というのは、iが指す要素を削除してしまうと、iは何も指していない状態になってしまうからです。
この状態では、インクリメントによって次の要素へ進めることはできません。幸いなことに、要素を削除するerase関数はこのことを心得ていて、戻り地で次の要素を指す値を返してくれます。まとめると「要素を削除する場合はerase関数の戻り値をiに代入し、そうでなければ通常通りiをインクリメントする」というプログラムを書く必要があるということで、実際に上記のプログラムはそのように書かれています。

さて、ようやくActorListが空かどうかを判定できます。MainGameScene.cppに戻り、MainGameScene::Update関数に次のプログラムを追加してください。
