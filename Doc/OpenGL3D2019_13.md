...
{
public:
  JizoActor(const Mesh::MeshPtr& m, const glm::vec3& pos, int id,
    MainGameScene* p);
...
* コンストラクタ.
*/
JizoActor::JizoActor(const Mesh::MeshPtr& m, const glm::vec3& pos, int id,
  MainGameScene* p) :
...
+      meshBuffer.GetMesh("JizoStatue"), position, i, this);
...
+    if (attackTimer > 0.05f && stateTimer < 0.6f) {
+      if (!attackCollision) {
